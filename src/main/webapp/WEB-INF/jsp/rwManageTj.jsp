...
<script type="text/javascript">
	...
	//任务状态
	function formateRwShenhe(val, row) {
		if (val == 0) {
	...
	function newRw() {
		...
		//加载责任人
		$("#rwZrId").val('${currentUser.realName }');
		$("#rwZrIdHidden").val('${currentUser.id }');
		//任务分配人
		$("#rwFpId").combobox({
			valueField:'id',
			textField:'realName',
	...
	function openRwModifyTab(){
		 ...
		 if (row.state == 0) { //未签收可以修改
			 $.ajax({
				 type:"POST",
				 url:"${pageContext.request.contextPath}/rw/findGrRwById?id="+row.id,
	                         ...
	                         if (result.success) {
	                        	 if (result.isEdit) { //可以修改任务
		                    		...
		                    		$("#rwFpId").combobox({
										...
										required: true,
										onLoadSuccess:function() {
											$(this).combobox("select",result.rw.rwFpId);
										}
									});
		                    		//加载参与人
		                    		$("#cy").combobox({
...
<table id="dg" title="任务管理" 
   ...
   url="${pageContext.request.contextPath}/rw/listGeRen" fit="true" toolbar="#tb">
   <thead>
   	<tr>
   		...
   		<th field="shenhe" width="50" align="center" formatter="formateRwShenhe">状态</th>
		...
		<table>
			...
			<tr>
				...
				<td style="width: 190px;">
					...
					<input id="rwZrId" readonly="readonly" type="text" style="width:180px;"  />
					<input id="rwZrIdHidden" type="hidden" name="rwZrId" /> 
				</td>
				<td style="width: 110px;" align="center">任务分配人</td>
				<td style="width: 150px;">
					<input editable="false" style="width: 96px;"  type="text" id="rwFpId" name="rwFpId" class="easyui-combobox" value="请选择分配人" />
				</td>
			</tr>
			<tr><td colspan="4" height="10px"></td></tr>
			<tr>

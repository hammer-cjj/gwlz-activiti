...
<script type="text/javascript">
	...
	function formateOperator(val, row) {
		return '<a href="#" onclick="completeDb('+row.id+')" class="rwDbBtn">完成督办 </a>';

	}
	
	//完成督办
	function completeDb(id) {
		$.ajax({
			type:"post",
			url: "${pageContext.request.contextPath}/rw/completeDb",
			data:{'id':id},
			dataType:"json",
			success:function(result) {
				if (result.success) {
					$.messager.alert("系统提示","完成督办成功！");
					$("#dg").datagrid("reload");
				} else {
					$.messager.alert("系统提示","完成督办失败！");
				}
			}
		});
	}
	...
	function newRw() {
		...
		url = '${pageContext.request.contextPath}/rw/saveBw';
		//加载责任人
		$("#zr").combobox({
...
<table id="dg" title="备忘任务管理" 
   fitColumns="true" pagination="true" rownumbers="true" singleSelect="true" scrollbarSize="0"
   url="${pageContext.request.contextPath}/rw/listBw" fit="true" toolbar="#tb">
   <thead>
   	<tr>
...
$("#dg").datagrid({
	onLoadSuccess:function(result){
		$(".rwDbBtn").linkbutton({text:"完成督办", plain:true});
